...
int func(vector<int> height){
    ...
    int mx = 0, high = height[i];
    vector<pair<int, int>> h1(height.size());
    vector<int> greater(height.size(), 0);
    for (int it = 0; it < height.size(); it++)
    {
        h1[it] = {height[it], it};
    }
    merge(h1, 0, height.size() - 1, greater);
    while (i <= j && greater[i] == 0)
    {
        i++;
    }
    high = height[i];
